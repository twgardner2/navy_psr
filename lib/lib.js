...
export function fitrep_tooltip(d) {
    ...
    var return_val = `
    Reporting Senior: ${d.rs_name}<br>
    Command: ${d.station}<br>
    Period: ${date_formatter(d.start_date)} to ${date_formatter(d.end_date)}<br>
    <br>
    Trait Average: ${d.trait_avg.toFixed(2)}<br>
    RSCA: ${d.rsca}<br>
    <br>
    ${d.n_ep ? `EP: ${d.n_ep}<br>` : ''}
    ${d.n_mp ? `MP: ${d.n_mp}<br>` : ''}
