...
export function get_command_dates(psr) {
    var commands = psr.map(fitrep => {
    ...
    var command_dates = unique_commands.map((command, i) => {
        ...
        var fitreps_this_command = psr.filter(fitrep => fitrep.station == command);

        // Reduce FITREPs this command down to earliest start_date
        var first_start_date_this_command = fitreps_this_command.reduce(
            function(acc, cur, idx, array) {
                var return_val; 
        ...
        var last_end_date_this_command = fitreps_this_command.reduce(
            function(acc, cur, idx, array) {
                var return_val; 
