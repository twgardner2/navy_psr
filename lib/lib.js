...
export function get_dates_for_values_of_column(psr, col, regex = new RegExp('.*', 'g'), regex_col = 'rpt_type') {
    ...
    var unique_values_of_col = [...new Set(filtered_fitreps.map(fitrep => fitrep[col]))];
    ...
    var dates = unique_values_of_col.map((value, i) => {
        ...
        var fitreps_this_value_of_col = filtered_fitreps.filter(fitrep => fitrep[col] == value);
...
// Interaction
export function update_fitrep_highlight(e, d) {

}
export function clear_fitrep_highlight(element_to_clear) {
