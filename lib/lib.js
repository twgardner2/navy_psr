...
export function get_command_dates(psr, regex) {
    ...
    var command = reg_fitreps.map(fitrep => {
        return fitrep.station;
    });
    var unique_command = [... new Set(command)];


    // Map over command, returning earliest start_date for each
    var command_dates = unique_command.map((command, i) => {
...
export function get_reporting_senior_dates(psr, regex) {
    ...
    // Get unique array of reporting seniors
    var reporting_seniors = reg_fitreps.map(fitrep => {
        return fitrep.rs_name;
    });
    var unique_reporting_seniors = [... new Set(reporting_seniors)];


    // Map over reporting_seniors, returning earliest start_date for each
    var command_dates = unique_reporting_seniors.map((reporting_senior, i) => {

        // FITREPs for this reporting senior
        var reg_fitreps_this_reporting_senior = reg_fitreps.filter(fitrep => fitrep.rs_name == reporting_senior);

        // Reduce FITREPs this reporting senior down to earliest start_date
        var first_start_date_this_reporting_senior = reg_fitreps_this_reporting_senior.reduce(
            function(acc, cur, idx, array) {
                var return_val; 
        ...
        // Reduce FITREPs this reporting senior down to latest end_date
        var last_end_date_this_reporting_senior = reg_fitreps_this_reporting_senior.reduce(
            function(acc, cur, idx, array) {
                var return_val; 
        ...
        // Return results
        return({'reporting_senior': reporting_senior,
                'start': first_start_date_this_reporting_senior,
                'end': last_end_date_this_reporting_senior});
...
export function fitrep_tooltip(d) {
    ...
    var return_val = 
    `
    <strong>Period:</strong> ${date_formatter(d.start_date)} to ${date_formatter(d.end_date)}<br>
    ...
    <br>
    <strong>Trait Average:</strong> ${d.trait_avg ? d.trait_avg.toFixed(2) : 'n/a'}<br>
    <strong>RSCA:</strong> ${d.rsca ? d.rsca : 'n/a'}<br>
    <strong>Delta:</strong> ${delta} <br>
    <br>
    <table>
        <tr>
            <td>${d.prom_rec.toUpperCase() == 'SP' ? '<strong>SP</strong>' : 'SP'}</td>
            <td>${d.prom_rec.toUpperCase() == 'PR' ? '<strong>PR</strong>' : 'PR'}</td>
            <td>${d.prom_rec.toUpperCase() == 'P'  ? '<strong>P</strong>'  : 'P'}</td>
            <td>${d.prom_rec.toUpperCase() == 'MP' ? '<strong>MP</strong>' : 'MP'}</td>
            <td>${d.prom_rec.toUpperCase() == 'EP' ? '<strong>EP</strong>' : 'EP'}</td>
        </tr>
        <tr>
            <td>${d.prom_rec.toUpperCase() == 'SP' ? `<strong>${d.n_sp}</strong>` : `${d.n_sp}`}</td>
            <td>${d.prom_rec.toUpperCase() == 'PR' ? `<strong>${d.n_pr}</strong>` : `${d.n_pr}`}</td>
            <td>${d.prom_rec.toUpperCase() == 'P'  ? `<strong>${d.n_p}</strong>`  : `${d.n_p}`}</td>
            <td>${d.prom_rec.toUpperCase() == 'MP' ? `<strong>${d.n_mp}</strong>` : `${d.n_mp}`}</td>
            <td>${d.prom_rec.toUpperCase() == 'EP' ? `<strong>${d.n_ep}</strong>` : `${d.n_ep}`}</td>
        </tr>
    </table>
    `
