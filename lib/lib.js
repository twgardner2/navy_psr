...
export function fitreps_same_rs_new_rank(psr) {
  var fitreps_by_rs = d3.group(psr, (d) => d.rs_name);
...
export function make_bars(
  data,
  parent_g,
  time_scale,
  ref_to_highlight = null,
  bar_color = "lightgrey",
  font_color = "black"
) {
  ...
  var bars = g
    ...
    .on("mouseover", function (event, d) {
      update_highlight_element(event, d, ref_to_highlight);
    })
    .on("mouseleave", function () {
      clear_fitrep_highlight(ref_to_highlight);
    });
