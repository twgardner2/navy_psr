...
module.exports.fields = {
    ...
    months: {
        ...
        calculated: {
            ...
            calculate: (d3Row) => {
                let date1 = d3Row
                    .select('td[data-key="start_date"] input')
                    .node()._flatpickr.selectedDates[0];
                let date2 = d3Row.select('td[data-key="end_date"] input').node()
        ...
        calculated: {
            ...
            calculate: (d3Row) => {
                let values = this.fields.trait_avg.calculated.watch.map((key) =>
                    Number(
                        d3Row.select(`td[data-key="${key}"] input`).node().value
                    )
                );
                ...
                let sum = 0;
                values.forEach((count, i) => (sum += (i + 1) * count));
    ...
    sum_group: {
        name: 'sum_group',
        type: 'number',
        display: 'Summary Reporting Group Avg',
        pdf: {
            parentColumn: 'Averages',
        },
    },
